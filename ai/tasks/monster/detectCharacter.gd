...
func _setup() -> void:
	...
	blackboard.set_var(BBVariable.    RootScale, [
		shapeCast3D.          scale.x
		, rayCast3D.target_position.x            ]);
	pass;
...
func _enter() -> void:
	...
	alreadyDetect = blackboard.get_var      (
					BBVariable.AlreadyDetect);  rootScale = blackboard.get_var  (
															BBVariable.RootScale);
	if (alreadyDetect == false):
		var scaleVector: Vector3 = Vector3(rootScale[0] * 1, 1, rootScale[0] * 1);
		shapeCast3D.scale = scaleVector; rayCast3D.target_position.x = rootScale[1] * 1;
...
#Projected vector on XZ of CurrentCharacter To TargetCharacter#
				...
				var targetCharacterAngle :  float  = Helper.    StandardizeDegree(
					rad_to_deg(
					targetCharacterVector.signed_angle_to(Vector3(1, 0, 0), Vector3.DOWN))
				);
				rayCast3D.position = (
				rayCast3D.position   );
				if (alreadyDetect == false):
...
func PatrollDetection(targetCharacterAngle: float, targetCharacter: Object) -> Status:
	...
	rayCast3D.   force_raycast_update   ()  ;
	if (
	rayCast3D.                               get_collider_rid()
	==                                targetCharacter.get_rid()):
		blackboard.set_var(BBVariable.TargetCharacter,
									  targetCharacter);
		blackboard.set_var(BBVariable.SeeingAngle    ,
			Helper.StandardizeDegree(   realAngle   ));
...
func   ActionDetection(            targetCharactedAngle: float, targetCharacter: Object) -> Status:
	...
	blackboard.set_var(BBVariable.TargetCharacter ,
								  targetCharacter);
	blackboard.set_var(BBVariable.SeeingAngle     ,
		Helper.StandardizeDegree ( targetCharactedAngle ));
	return SUCCESS;
