...
public partial class Cyclops : ElementalMonster
{
    ...
    {
        CreateAbility(   nameof(MiniThrowingRock), targetMainCharacter);
    }
    ...
    public override void CreateAbility(string abilityName, MainCharacter targetMainCharacter)
    {
       ...
       .Instantiate<
        ...
        if (       0 <=
            distance
        &&  distance <= AbilityStats.ActiveRange.Short )
        {
            tempAbility.DamageRatio = 0.8f;
        }
        else
        if (            AbilityStats.ActiveRange.Short
          < distance
        ...
        if (            AbilityStats.ActiveRange.Medium
          < distance
        ...
        if (            AbilityStats.ActiveRange.Long
          < distance
