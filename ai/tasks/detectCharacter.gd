...
var BBSeeingAngle: StringName = "SeeingAngle"; var BBTargetCharacter: StringName = "TargetCharacter"; var BBAlreadyDetect: StringName = "AlreadyDetect";
...
func _enter() -> void:
	...
	if (alreadyDetect == false):
		shapeCast3D.scale = Vector3(2, 1, 2); rayCast3D.target_position.x = 0.5;
		var tween : Tween = currentCharacter.create_tween();
		tween.tween_property(eyeSight, "scale", Vector3(2, 1, 2), 0.5);
		tween.set_ease (Tween. EASE_OUT );
		tween.set_trans(Tween.TRANS_CIRC);
	else:
		shapeCast3D.scale = Vector3(4, 1, 4); rayCast3D.target_position.x = 1.0;
		var tween : Tween = currentCharacter.create_tween();
		tween.tween_property(eyeSight, "scale", Vector3(4, 1, 4), 0.5);
		tween.set_ease (Tween. EASE_OUT );
		tween.set_trans(Tween.TRANS_CIRC);
	pass;
...
func PatrollDetection(targetCharacterAngle: float, targetCharacter: Object) -> Status:
	...
	if (rayCast3D.get_collider_rid() == targetCharacter .get_rid()):
		blackboard.set_var(
		             BBTargetCharacter, targetCharacter);
