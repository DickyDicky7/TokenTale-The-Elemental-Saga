...
{
    public Quiver
           Quiver { get; private set; }
    public   Boot
             Boot { get; private set; }
    public PowerupsGenerator  PowerupsGenerator { get; private set; }
    public List<   HealthJar>     HealthJarList { get; private set; } = [];
    public List<ElementalJar> @ElementalJarList { get; private set; } = [];
    ...
    public override void _Input(InputEvent @event)
    {
                    ...
                    inputEventKey)
        {
            if     (inputEventKey.Pressed         )
            {
                ...
                {
                    if (this.MainCharacter.CurrentHealth
                    <   this.MainCharacter.    MaxHealth)
                    {
                        float neededAmount = this.MainCharacter.    MaxHealth
                                           - this.MainCharacter.CurrentHealth;
