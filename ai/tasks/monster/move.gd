...
func _setup() -> void:
	...
	flipSprite3D      = agent.get_node(
PathFlipSprite3D);
	navigationAgent3D = agent.get_node(
PathNavigationAgent3D);
	hurtbox3D         = agent.get_node(
PathHurtbox3D);
...
func _tick(_delta: float) -> Status:
	...
	if (distanceToTargetXZ
	<=  navigationAgent3D.target_desired_distance):
		return SUCCESS;
	else:
		var nextDestination: Vector3 = (
