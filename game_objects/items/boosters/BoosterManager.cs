...
public partial class BoosterManager : Node
{
    ...
    [Signal]
    public delegate void     SwordSrcollChangedEventHandler(float newSwordCooldown         ,
                                                            float newSwordBonusDamageRatio);
    [Signal]
    public delegate void       BowScrollChangedEventHandler(float newBowCooldown         ,
                                                            float newBowBonusDamageRatio);
    [Signal]
    public delegate void ElementalScrollChangedEventHandler(float newElementalCoolDown         ,
                                                            float newElementalBonusDamageRatio);

    private MainCharacter MainCharacter { get; set; }
    public List<bool>           HeartStatusList = [];
    public List<bool>     EnergyStoneStatusList = [];
    public List<bool>     SwordScrollStatusList = [];
    public List<bool>       BowScrollStatusList = [];
    public List<bool> ElementalScrollStatusList = [];
    ...
    public void TakeBooster(Booster booster)
    {
        ...
        if (booster.GetType() == typeof(    EnergyStone))
        {
                EnergyStoneStatusList[booster.Key] = true;
           ApplyEnergyStone();
        }
        else
        if (booster.GetType() == typeof(    SwordScroll))
        {
                SwordScrollStatusList[booster.Key] = true;
           ApplySwordScroll();
        }
        else
        if (booster.GetType() == typeof(      BowScroll))
        {
                  BowScrollStatusList[booster.Key] = true;
             ApplyBowScroll();
        }
        else
        if (booster.GetType() == typeof(ElementalScroll))
        {
    ...
    private void ApplyHeart()
    {
        ...
        this.MaxHealth                     =
               BoosterStats.GetInstance()
            .MaxHealthStats[HeartCollected]
            .MaxHealth;
        this.@MainCharacter
            .MaxHealth
      = this.MaxHealth;
        this.EmitSignal(BoosterManager.SignalName.HeartChanged,
        this.MaxHealth);
    }

    private void ApplyEnergyStone()
    {
        ...
        this.MaxEnergy                           =
            ...
            .MaxEnergy;
        this.EmitSignal(BoosterManager.SignalName. EnergyStoneChanged,
        this.MaxEnergy );
    }
    ...
    private void ApplySwordScroll()
    {
        ...
        Record.SwordKnowledgeInfo info = BoosterStats.GetInstance().SwordProficiency[SwordScrollCollected]                                                     ;
        this.SwordCoolDown         =
        info.     CoolDown         ;
        this.SwordBonusDamageRatio =
        info.     BonusDamageRatio ;
        this.EmitSignal(
  BoosterManager.SignalName.SwordSrcollChanged,
    ...
    private void ApplyBowScroll()
    {
        ...
        Record.BowKnowledgeInfo info = BoosterStats.GetInstance().BowProficiency[BowScrollCollected]                                                   ;
        this.BowCoolDown         =
        info.   CoolDown         ;
  ...
  BoosterManager.SignalName.BowScrollChanged,
    ...
    private void ApplyElementalScroll()
    {
        ...
        Record.ElementalKnowledgeInfo info = BoosterStats.GetInstance().ElementalProficiency[ElementalScrollCollected]                                                         ;
